...
router.all(
  ...
  catchAsyncErrors(async (req, res, next) => {
        ...
        if (existingUserPrice) {
          ...
          if (existingRetrUserPrice) {
            ...
            const newPriceData = {
              user_id: retr_user_id,
              country_code: country_code,
              basic_sms_cost:  margin,
              p2a_conv_cost:  margin,
              a2p_conv_cost:  margin,
              a2p_single_sms_cost:  margin,
              is_active: 1,
              bot_id: bot_id,
              is_frozen: 0,
              created_date: todayDateTime(),
            ...
            return res.status(200).json({
              success: true,
              message: "Pricing updated successfully for single country (new entry)",
            });
          }
        }
        else {
          return res.status(404).json({ success: false, message: "entry for provided Country code not found " });
        }
    ...
    else if (resdata.method === "add_single_country_price_smsvoicewp") {
  ...
  WHERE country_code = ? AND user_id = ?;
    ...
    else if (resdata.method === "add_all_country_price_smsvoicewp") {
      const { margin, country_code, parent_type, client_type, user_id } = resdata;
