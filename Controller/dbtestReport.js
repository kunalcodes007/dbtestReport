...
FROM tbl_sms_summary
JOIN tbl_users ON tbl_sms_summary.user_id = tbl_users.id
...
LEFT JOIN 
    (
        ...
        FROM
            tbl_users u
        LEFT JOIN
            tbl_whatsapp_billing_summary wb ON u.id = wb.user_id
        WHERE
...
FROM tbl_voice_summary
JOIN tbl_users ON tbl_voice_summary.user_id = tbl_users.id
WHERE date BETWEEN ? AND ?
GROUP BY user_id
`;
...
FROM tbl_email_summary
JOIN tbl_users ON tbl_email_summary.user_id = tbl_users.id
WHERE date BETWEEN ? AND ?
GROUP BY user_id
`;
